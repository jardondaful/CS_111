...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	int n = pthread_mutex_init(&(hash_table->m), NULL);
	if (n != 0) {
	  exit(n);
	}
                             ...
                             uint32_t value)
{
  ...
  if (pthread_mutex_lock(&(hash_table->m)) != 0)
	  exit(EINVAL);
	...
	if (list_entry != NULL) {
	  ...
	  if (pthread_mutex_unlock(&(hash_table->m)) != 0)
	    exit(EPERM);
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	int n = pthread_mutex_destroy(&(hash_table->m));
	if (n != 0) {
	  exit(n);
	}
