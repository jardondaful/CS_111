...
void write_block_bitmap(int fd) {
  ...
  u8 block_bitmap_arr[BLOCK_SIZE] = {0};

  block_bitmap_arr[0] = 0xFF;
  block_bitmap_arr[1] = 0xFF;
  block_bitmap_arr[2] = 0b01111111;
  block_bitmap_arr[127] = 0b10000000;
  for (int i = 3; i < BLOCK_SIZE; i++) {
    if (i < 127)
      block_bitmap_arr[i] = 0x00;
    else if (i > 127)
      block_bitmap_arr[i] = 0xFF;
  }

  if (write(fd, block_bitmap_arr, BLOCK_SIZE) != BLOCK_SIZE)
    errno_exit("write");
}
...
void write_inode_bitmap(int fd) {
  ...
  inode_bitmap_arr[0] = 0xFF;
  inode_bitmap_arr[1] = 0b00011111;
  for (int i = 2; i < BLOCK_SIZE; i++) {
    if (i < 16)
      inode_bitmap_arr[i] = 0x00;
    else
      inode_bitmap_arr[i] = 0xFF;
  }

  if (write(fd, inode_bitmap_arr, BLOCK_SIZE) != BLOCK_SIZE)
    errno_exit("write");
}
